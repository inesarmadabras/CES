        ...
        <div class="row">
            <div class="col-md-1">
                <div class="upvote topic upvote-disabled" data-post="{{ $post->id }}">
                    ...
                    <span class="count text-center">{{ $post->votes->sum('value') }}</span>
            ...
            <div class="col-md-10">
                ...
                <p style="color: darkgrey; font-size: 12px;">
                    <i class="glyphicon glyphicon-user" style="padding-right: 5px;"></i>publicado por {!!  link_to_route('profile_path', $post->user->name, $post->user->name) !!}
                    ...
                    <i class="glyphicon glyphicon-comment" style="padding-left: 15px;"></i> <a href="{{ action('PostsController@show', [$post->id]) }}">{{ $post->comments->count() }} Comentários</a>
                    @can('update-post', [$post, $isModerator])
                        <i class="glyphicon glyphicon-pencil" style="padding-left: 15px;"></i> <a href="{{ action('PostsController@edit', $post->id) }}">Editar</a>
                    @endcan
