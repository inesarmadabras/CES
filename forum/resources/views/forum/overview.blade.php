        ...
        <div class="col-md-3">
            {{ Form::open(['route' => 'forum', 'method' => 'GET']) }}
                <div class="form-group">
                    {{ Form::text('search', $search ?? null, ['class' => 'form-control', 'placeholder' => 'Pesquisar por tópicos...']) }}
                </div>
            {{ Form::close() }}

            <a class="btn btn-success btn-block" href="{{ route('threads.create') }}">Criar Tópico</a>
        ...
        <div class="col-md-9">
                        ...
                        <div class="panel-heading thread-info">
                            ...
                            <div class="thread-info-author">
                                @if (count($thread->replies()))
                                    ...
                                    <a href="{{ route('profile', $lastReply->author()->username()) }}" class="thread-info-link">{{ $lastReply->author()->name() }}</a> respondeu
                                    {{ $lastReply->createdAt()->diffForHumans() }}
                                @else
                                    <a href="{{ route('profile', $thread->author()->username()) }}" class="thread-info-link">{{ $thread->author()->name() }}</a> publicou
            ...
            @else
                <div class="alert alert-info text-center">
                    ...
                    <a href="{{ route('threads.create') }}" class="alert-link">Crie um novo</a>
                </div>
            @endif
        </div>
    </div>
