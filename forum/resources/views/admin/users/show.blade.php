            ...
            <div class="col-md-3">
                <div class="panel panel-default">
                    <div class="panel-body">
                        <a class="btn btn-default btn-block" href="{{ route('profile', $user->username()) }}">Ver Perfil</a>

                        @can(App\Policies\UserPolicy::BAN, $user)
                            @if ($user->isBanned())
                                <button type="button" class="btn btn-warning btn-block" data-toggle="modal" data-target="#unbanUser">Desbloquear Utilizador</button>
                            @else
                                <button type="button" class="btn btn-warning btn-block" data-toggle="modal" data-target="#banUser">Bloquear/Banir Utilizador</button>
                            @endif
                        @endcan

                        @can(App\Policies\UserPolicy::DELETE, $user)
                            <button type="button" class="btn btn-danger btn-block" data-toggle="modal" data-target="#deleteUser">Apagar Utilizador</button>
                        @endcan
                    </div>
                </div>

                <p style="text-align:center"><a href="{{ route('admin') }}"><i class="fa fa-arrow-left"></i> Voltar</a></p>
            </div>
            <div class="col-md-9">
